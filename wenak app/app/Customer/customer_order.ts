...
router.post("/send_order", function(req: any, res: any, next: any) {
    //   console.log("sarame");
    //   User.findById(req.user_id, { password: 0 }, function(err: any, user: any) {
    //     if (err)
    ...
    console.log(myorder);
    if (!myorder) return res.status(400).json({ msg: "problem in order" });
    else if (myorder) {
      // console.log(myorder);
      Order.create({
        // user_id:id,
        ...
        res.json(console.log(" successfully")).catch((err: any) => {
          res.send("error" + err);
        });
      });
    } else {
