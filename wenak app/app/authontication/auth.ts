...
const client = require("twilio")(
    "ACec49a638f1063d9909e19325c1cd3247",
    "190ffee0620ff3a9b93d93d562eeea78"
);
router.use(pino);
...
router.post("/register", [
  ...
  User.findOne({mobilenum}, (err: any, user: any) => {
    if (err) {
    ...
    User.create({
      ...
      res.status(200).send({ auth: true, hello});
...
router.post("/login", function(req: any, res: any) {
      User.findOne({ mobilenum: req.body.mobilenum }, function(err: any, user: any) {
        ...
        res.status(200).send({ auth: true, token, type});
      });
    });
